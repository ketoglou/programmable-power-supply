        ...
        try:
            while getattr(self.t, "do_run", True) and self.ser != None:
                ...
                if voltage != None and current_voltage != None:
                    voltage = ((10000.0 + 1600.0) / 1600.0) * \
                        float(voltage)  # Vs = ((R1+R2)/R2)*Vout
                    self.voltage_label.set_label("{0:.6f} V".format(voltage))
                    # We divide with 4.9 because thats the factor the amplifier amplifies the voltage
                    current_voltage = float(current_voltage) / 4.9
                    # We divide the real voltage drop we measure from amplifier with the resistor
                    current = current_voltage / 1000.0
                    # The real current at load is the current we find above in the resistor*5000 as the datasheet of LT3081 says
                    current = current*5000.0
...
class SerialPort:
    ...
    def open_serial(self, s_port):
        try:
            self.ser = serial.Serial(port=s_port, baudrate=19200, bytesize=serial.EIGHTBITS, 	parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE,
                                     timeout=1, xonxoff=False, rtscts=False, write_timeout=None, dsrdtr=False, inter_byte_timeout=None, exclusive=None)
            self.ser.isOpen()
        ...
        try:
            ...
            if command == "R2":
                respond = self.ser.readline().decode("utf-8") + \
                    self.ser.readline().decode("utf-8") + self.ser.readline().decode("utf-8")
            else:
                respond = self.ser.readline().decode("utf-8")
            return respond
        except:
...
class UIWindow:
    ...
    def start_thread(self):
        self.thread = threading.Thread(target=VoltageCurrent_thread, args=(
            self.serial, self.voltage_label, self.current_label))
        self.thread.start()
    ...
    def voltage_set(self, widget, data=None):
        try:
            ...
            if voltage >= 0.0 and voltage <= 5.0:
                # According to LT3081 datasheet with this method we calculate the resistor
                r = voltage / 0.00005
    ...
    def current_set(self, widget, data=None):
        try:
            ...
            if Ilimit >= 0.0 and Ilimit <= 1.5:
                # Ilimit is the current we set as limit,Irm is the current the amplifier read,Irm is 1/5000 of Iload(Ilimit)
                Irm = Ilimit / 5000.0
                # Vrm is the differential voltage that the mcu reads,the real is 1000*Irm,but is amplified by 4.9
                Vrm = 1000*Irm*4.9
                self.destroy_thread()
